...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:disco="http://schemas.xmlsoap.org/disco/" xmlns:scl="http://schemas.xmlsoap.org/disco/scl/" xmlns:schema="http://schemas.xmlsoap.org/disco/schema" xmlns:xsd="http://www.w3.org/2001/XMLSchema/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance/" xmlns:ms="urn:schemas-microsoft-com:xslt" xmlns:s="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="disco scl ms s xsi xsd">
	...
	<xsl:template match="/">
		<h1 class="listpage" _locID="L_string01_Text">Страница обнаружения</h1>
		<p class="listpage" _locID="L_string02_Text">Этот документ обнаружения представляет собой 
			группу веб-служб (например, service.wsdl), группу Dataset (например, dataset.xsd) и другие 
			документы обнаружения (например, group.disco), которые могут включать друг друга. </p>
		<h3 _locID="L_string03_Text">Состав группы веб-служб:</h3>
		<table class="listpage" borderColor="#ffffff" cellSpacing="0" cellPadding="0" rules="rows" width="100%" align="center" frame="void">
			        ...
			        <tr valign="center" align="left">
				        <td class="tbltext">
					        <table borderColor="#ffffff" cellSpacing="0" cellPadding="3" rules="rows" width="100%" border="1" frame="void">
        						<tr>
		        					<td class="header" _locID="L_string04_Text">Службы</td>
		        			    </tr>
						            ...
						            <tr vAlign="center">
							            <td class="tbltext">
							                <xsl:value-of select="@ref"/><br/>
							                    <a _locID="L_string05_Text"><xsl:call-template name="writeHrefAttrib"> <xsl:with-param name="url" select="@ref"/></xsl:call-template>Просмотр службы</a>
									             <a _locID="L_string06_Text"><xsl:call-template name="writeHrefAttrib"> <xsl:with-param name="url" select="@docRef"/></xsl:call-template>Просмотр документации</a>
									    </td>
									</tr>
    				            </xsl:for-each>
    				        </table>
				...
				<xsl:when test="disco:discovery/schema:schemaRef">
					<tr valign="center" align="left">
						<td class="tbltext">
					        <table borderColor="#ffffff" cellSpacing="0" cellPadding="3" rules="rows" width="100%" border="1" frame="void">
								<tr>
									<td class="header" _locID="L_string07_Text">Объекты DataSet</td>
								</tr>
            					<xsl:for-each select="disco:discovery/schema:schemaRef">
                					<xsl:sort select="@ref"/>
			        			    <tr valign="center">
					        		    <td class="tbltext"><xsl:value-of select="@ref"/><br/><a _locID="L_string08_Text"><xsl:call-template name="writeHrefAttrib"> <xsl:with-param name="url" select="@ref"/></xsl:call-template><BR/>Просмотр схемы</a>
									    </td>
								    </tr>
            					    </xsl:for-each>
    				        </table>
				...
				<xsl:when test="disco:discovery/disco:discoveryRef">
					<tr valign="center" align="left">
						<td class="tbltext">
					        <table borderColor="#ffffff" cellSpacing="0" cellPadding="3" rules="rows" width="100%" border="1" frame="void">
								<tr>
									<td class="header" _locID="L_string09_Text">Документы обнаружения</td>
								</tr>
            					<xsl:for-each select="disco:discovery/disco:discoveryRef">
                					<xsl:sort select="@ref"/>
			        			    <tr valign="center">
					        		    <td class="tbltext"><xsl:value-of select="@ref"/><br/><a _locID="L_string10_Text"><xsl:call-template name="writeHrefAttrib"> <xsl:with-param name="url" select="@ref"/></xsl:call-template><BR/>Просмотр документа обнаружения</a>
									    </td>
								    </tr>
            					    </xsl:for-each>
    				        </table>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<tr valign="center">
						<td class="tbltext" _locID="L_string11_Text">Этим документом обнаружения веб-ссылки не определяются.</td>
					</tr>
				</xsl:otherwise>
			</xsl:choose>
		</table>
